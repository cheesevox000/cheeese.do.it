...
public class CLI {
    ...
    private static String help = "Help: \n"
	    ...
	    + "java -jar LAB2.jar -r <<user_CSV_file>> -delete <<email>> -mode <<delete_mode>> -<<order>> <<new_user_CSV_file>>: Read the user csv file, delete the users with email and save the tree strucutre with <<order>> into csv file, <<order>> is one in [lnr, nlr, lrn], <<delete_mode>> is one in [merge-right, merge-left, copy-right, copy-left]\n"
	...
	try {
		...
		String[] parameterID = Arrays.copyOfRange(args, 0, args.length);
		Manageinout.inputCLI(parameterID);
